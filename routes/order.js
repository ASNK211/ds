...
router.post("/",verifyToken, async (req, res) => {
      ...
      console.log(orders.profit)
      if(orders.active === "false") {
          return  res.status(400).json({massage:"You haven’t enough profit and balance"});
      } else if (orders.balance + orders.profit < amount) {
        return  res.status(400).json({massage:"Your profit and balance is less or not equal to the amount to be withdrawn"});
      } else {
